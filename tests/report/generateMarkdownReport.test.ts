...
describe("renderMarkdownReport", () => {
  ...
  it("renders a blocker under Must Fix with every PRD field + trust signals", () => {
    ...
    expect(mustFix).toContain("**Reported by:** 2 reviewers, 1 finding merged (reviewer_a, reviewer_b)");
  ...
  it("lists every agreeing reviewer when a large roster converges on one finding", () => {
    ...
    expect(section(md, "Must Fix Before Human Review")).toContain(
      `**Reported by:** 5 reviewers, 1 finding merged (${agents.join(", ")})`,
    );
  });
  ...
  it("is deterministic and ends with a trailing newline", () => {
    ...
    expect(a).toBe(b);
    expect(a.endsWith("\n")).toBe(true);
    expect(a).toContain("_Generated by pr-war-room v0.1.0 at 2026-01-01T00:00:00.000Z (run test-run)._");
  });
